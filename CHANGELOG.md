...
- **Google OAuth sign-in/sign-up (2026-08-14)**
  - Backend (`apps/api/src/modules/auth/`): `passport-google-oauth20` strategy (registered only when `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` are set — otherwise the Google endpoints answer `503 AUTH_GOOGLE_NOT_CONFIGURED` and email/password auth is untouched).
  - Endpoints: `GET /auth/google` (redirect to Google consent, signed `state` carries the post-login path — HMAC-keyed with the JWT access secret, blocks open redirects), `GET /auth/google/callback` (passport exchange → browser redirected back to the SPA with a short-lived one-time code — never a token), `POST /auth/google/exchange` (SPA exchanges the code for the standard token pair; single-use, 60s TTL, in-memory `GoogleCodeService`).
  - Account resolution in `AuthService.googleLogin`: existing Google user → sign in; matching *verified* Google email on a credentials account → account linking; otherwise create the user + their own tenant (OWNER), mirroring register. Google-only accounts have no password (schema relaxed) and are rejected by password login with a generic 401. An unverified email claim is never allowed to take over an existing account.
  - `User` schema: `passwordHash` now optional, `googleId` (unique, sparse). `UserRepository`: `findByGoogleId`, `linkGoogleId`, `create` accepts optional `passwordHash`/`googleId`/`avatarUrl` (new users inherit their Google photo as avatar).
  - New error codes `AUTH_GOOGLE_NOT_CONFIGURED`, `AUTH_GOOGLE_EMAIL_REQUIRED`, `AUTH_GOOGLE_CODE_INVALID`; config vars `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_CALLBACK_URL`, `WEB_URL` (documented in `.env.example`).
  - Frontend: "Continue with Google" (login) / "Sign up with Google" (register) buttons, `GoogleCallbackPage` at `/auth/google/callback` (code exchange → sign-in → `next` redirect), new multicolor `Google` icon in `apps/web/src/shared/components/icons.tsx`.
  - Tests: 18 new unit tests (AuthService googleLogin — create/link/reject-unverified/disabled/no-email, issueSessionForPrincipal, signed-state round-trip/tamper/open-redirect; GoogleCodeService single-use + TTL). Auth suite now 43/43; full API suite 277 passing.
- **Image persistence hardening + profile photo editor (2026-08-13)**
  - API storage no longer anchored to the process working directory: new `resolveStorageRoot()` (`apps/api/src/common/utils/storage-root.util.ts`, `STORAGE_ROOT` override documented in `.env.example`) is used by avatar storage and the documents storage/staging directories — uploaded images stay reachable across restarts regardless of launch directory (fixes images "disappearing" on refresh when the API is started from a different folder). Files keep living under `<api package>/storage/`.
  - `ImageEditor` component (`apps/web/src/shared/components/ImageEditor.tsx`): drag-to-position, scroll/slider zoom, 90° rotation (CW/CCW), aspect-locked crop with canvas export. Settings profile photo upload now routes every selection through the editor — crop, resize, and reposition before upload (square 512 px output).
  - New icons (`Crop`, `ZoomIn`, `ZoomOut`, `RotateCw`, `RotateCcw`, `Undo2`) in `apps/web/src/shared/components/icons.tsx`.
- **Multi-tenant organizations, team & invitations (prompt2.txt, 2026-08-11)**
  - Organization system built on the existing `Tenant`/`Membership` models (no new data layer): `OrganizationsModule` with `GET /organizations/me` (active org context + membership list), `POST /organizations` (create org → founder becomes OWNER, fresh token pair), `POST /organizations/switch` (server-verified membership before re-issuing tokens — a client can never switch to an org it has no membership in).
  - Team management (`GET/PATCH/DELETE /organizations/members`): OWNER/ADMIN-only list, role changes (only OWNER can grant OWNER; nobody can change their own role; the last OWNER/ADMIN cannot be demoted or removed), member removal.
  - Invitation flow: `Invitation` Mongoose schema (64-hex `crypto.randomBytes(32)` token, status pending/accepted/revoked, 7-day expiry, tenant-scoped indexes), `POST/GET/DELETE /organizations/invitations` (OWNER/ADMIN), public `GET /invitations/:token` + `POST /invitations/:token/accept`. Existing users must be logged in as the invited email (a bare link cannot grant membership to a stranger's account); new users create their account inline. Duplicate membership is structurally impossible (unique `(userId, tenantId)` index). Dev acceptance link (`devAcceptUrl`) is development-only — production email delivery deferred.
  - Roles: new `Role.OWNER`; registration + seed founder now get OWNER (was ADMIN); `TENANT_ADMIN_ROLES` includes OWNER so owners bypass project-membership checks like admins.
  - Project isolation hardening: `ProjectsService.addMember` now verifies the target user holds an organization membership before assignment — cross-organization project membership is rejected with `ORG_MEMBERSHIP_REQUIRED`.
  - Frontend: Team page (`/team`, sidebar entry desktop + mobile) with members table, invite form, pending-invitation list with copyable dev links and revocation; `InvitationAcceptPage` (`/invitations/:token`) handling existing-user login/accept and new-user signup/accept, then landing immediately in the invited org; organization switcher in the profile menu (fetches memberships on open, server-verified switch, fresh tokens); login/register pages accept a `?next=` redirect for the invitation flow.
  - Tests: 36 new `OrganizationsService` tests (context resolution, switching, team management authorization, invitation lifecycle, expiry/tampering, duplicate-membership prevention, "David cannot make himself owner") + 1 new project cross-org membership test. Full API suite: 257 tests passing. Typecheck/lint clean.
  - Member directory (`GET /organizations/directory`): OWNER/ADMIN/PROJECT_MANAGER read-only list of member names/emails/roles so project managers can assign project members without team-management powers; non-managers get `403`. Used by the Project Detail assignee picker (search + avatar). 2 new `OrganizationsService` tests (allowed roles, non-manager rejection) — 259 total.
  - Docs: new feature spec `docs/features/organizations.md`; authorization matrix updated with the `owner` column and team/directory rows; endpoint catalog and authentication docs aligned.
- **Profile photos (2026-08-08)**
  - `avatarUrl` on `User` schema/domain; `PATCH /api/v1/auth/me` (name update), `POST /api/v1/auth/me/avatar` (2 MB cap, JPEG/PNG/WebP, stored under `storage/avatars/<userId>.<ext>`, old file removed on replace), `DELETE /api/v1/auth/me/avatar`, public `GET /api/v1/auth/:userId/avatar` (cacheable, ObjectId URLs unguessable). New `AUTH_INVALID_AVATAR` error code.
  - Settings Profile tab: avatar preview, upload with client-side type/size validation, remove; avatar shown in the topbar profile menu, team member rows, and login/register responses.
